...
1. **Check Known Dead-Ends.** Scan the "Known Dead-Ends" section below. If your proof requires any of these patterns, sorry it immediately and move on:
   ...
   - `SMul k (End Y)` vs `SMul k (Y ⟶ Y)` instance diamond in `ModuleCat` — `End Y` (from `CategoryTheory.Endomorphism`) gets `SMul k` via `Linear.instAlgebraEnd.toSMul`, while `Y ⟶ Y` gets it via `ModuleCat.instSMulHom`. These are definitionally equal but syntactically different instances. `show`, `change`, `rfl`, and `congr` all fail to bridge them. Workaround: avoid mixing `End Y` and `Y ⟶ Y` types in the same goal; use `unfold End` early or work entirely in one type. Blocks `equivEndAlgEquiv` scalar preservation in MoritaStructural.
