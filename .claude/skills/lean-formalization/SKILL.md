...
13. **Subalgebra of End(E) instance resolution timeout.** When `A : Subalgebra k (Module.End k E)` and `S : Submodule A E`, synthesizing `Module A ↥S`, `SMulZeroClass A ↥S`, or `Module k ↥S` often times out at default heartbeats (20000). The instance chain goes through `Subalgebra → Ring → Semiring → SMul → Module` and is too deep. **Fix:** Use `set_option synthInstance.maxHeartbeats 200000` (with `set_option maxHeartbeats 400000`). Do NOT try to provide instances manually via `letI` — this creates instance diamonds where your explicit instance doesn't match the one Lean finds elsewhere, causing `IsSimpleModule` or `LinearEquiv` type mismatches.
