...
- **Latest update:** 2026-03-20 (wave 21 summary)
- **Notes:** 202/583 items sorry_free (34.6%). 94 sorry occurrences across 32 files. ~274 PRs merged across twenty-one proof waves. Four chapters at 100% formal completion: Ch3, Ch4, Ch7, Ch8. Wave 21 merged 24 PRs with +5 sorry_free items (best single-wave gain). Key completions: Theorem 5.14.3 (character formula via power sums), Lemma 5.25.3 dimension (GL₂ complementary series), hσ_inj (Theorem 9.2.1 block assignment injectivity). FRT hook-length formula induction step proved modulo two sub-lemmas. Sorry count rose from 84→94 due to new proof scaffolding (Dynkin forward direction, GL₂ conjugacy classes). Remaining work concentrated in Ch5 (50 sorries across 17 files) and Ch6 (32 sorries across 10 files). See `progress/summaries/wave-21-summary.md` for full details.
