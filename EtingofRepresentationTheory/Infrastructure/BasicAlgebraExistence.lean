...
The fullness condition `AeA = A` is essential: it ensures the functor is
faithful (every element of M lies in the A-span of eM) and full (A-linear
maps can be reconstructed from their restriction to eM). -/
